...
export function logPerf(metric) {
    try {
 ...
 */
export function readPerfLog(buildId) {
    try {
        ...
        const entries = content.trim().split('\n')
            ...
            .filter((e) => e !== null);
        if (buildId)
            return entries.filter((e) => e.buildId === buildId);
        return entries;
    }
    catch {
        return [];
 ...
 */
export function summarizePerf(buildId) {
    const entries = readPerfLog(buildId);
    if (entries.length === 0)
        return { buildId, entries: 0 };
