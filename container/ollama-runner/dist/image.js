...
async function comfyCheckpointAvailable(unetName) {
    try {
...
async function comfyWaitForImage(promptId) {
...
export async function generateImageComfyUI(prompt) {
...
async function listOllamaDiffuserImageModels() {
    try {
        const resp = await fetch(`${OLLAMADIFFUSER_HOST}/api/models`, {
...
export async function generateImageOllamaDiffuser(prompt, refImageBase64) {
    if (!(await ollamaDiffuserAvailable()))
        throw new Error('OllamaDiffuser is not running (port 8001).');
    const model = await pickImageModel();
    if (!model)
        throw new Error('No image model found in OllamaDiffuser.');
    ...
    if (!b64)
        throw new Error('OllamaDiffuser returned no image');
    return { buffer: Buffer.from(b64, 'base64'), model };
}
// ---------------------------------------------------------------------------
// Ollama backend (fallback)
// ---------------------------------------------------------------------------
export async function generateImageOllama(model, prompt) {
    ...
    if (!resp.ok)
        throw new Error(`Ollama image gen error: ${resp.status}`);
    const data = (await resp.json());
    const b64 = data.image ?? data.images?.[0] ?? data.response;
    if (!b64)
        throw new Error('No image in Ollama response');
    return { buffer: Buffer.from(b64, 'base64'), model };
}
// ---------------------------------------------------------------------------
...
export async function describeImages(images) {
    return Promise.all(images.map(async (img, i) => {
            ...
            const resp = await fetch(`${OLLAMA_HOST}/api/chat`, {
                ...
                body: JSON.stringify({
                    ...
                    messages: [
                        {
                            role: 'user',
                            content: 'Describe this image in detail: subject, appearance, clothing, colors, background, lighting, style. Be specific and thorough.',
                            images: [img],
                        },
                    ],
                    stream: false,
        ...
        .map((desc, i) => desc ? (images.length > 1 ? `Character ${i + 1}: ${desc}` : desc) : null)
        .filter((d) => d !== null));
}
export async function generateImage(ollamaModel, prompt, backend = 'auto', options) {
