...
async function describeImageVision(imageBase64) {
    try {
...
export function loadReferenceImages() {
    try {
        if (!fs.existsSync(LATEST_IMAGE_FILE))
            return null;
        const data = JSON.parse(fs.readFileSync(LATEST_IMAGE_FILE, 'utf-8'));
...
export function hasReferenceVideo() {
    try {
        if (!fs.existsSync(LATEST_VIDEO_FILE) || !fs.existsSync(LATEST_VIDEO_PATH))
            return false;
        const data = JSON.parse(fs.readFileSync(LATEST_VIDEO_FILE, 'utf-8'));
...
function transcodeToH264(input) {
    ...
    try {
        ...
        execSync(`ffmpeg -i "${tmpIn}" -vcodec libx264 -crf 23 -preset fast -acodec aac -movflags +faststart "${tmpOut}" -y`, { timeout: 120_000, stdio: ['pipe', 'pipe', 'pipe'] });
...
async function uploadImageToComfyUI(imageBuffer, filename = 'reference.jpg') {
...
function applyWorkflow(workflow, vars) {
...
// ComfyUI backend
// ---------------------------------------------------------------------------
async function comfyuiAvailable() {
    try {
...
export async function listComfyVideoModels() {
    try {
        ...
        for (const def of T2V_WORKFLOWS) {
            if (!available.has(def.checkpoint))
                continue;
            const baseName = def.label.replace(/\s*\(T2V\)/, '');
            if (!byCheckpoint.has(def.checkpoint))
                byCheckpoint.set(def.checkpoint, { baseName, modes: new Set() });
        ...
        return Array.from(byCheckpoint.values()).map(({ baseName, modes }) => modes.size > 1 ? `${baseName} (T2V + I2V)` : `${baseName} (${[...modes][0]})`);
    }
    catch {
        return [];
    }
}
async function comfyCheckpointAvailable(name) {
    try {
        const resp = await fetch(`${COMFYUI_HOST}/object_info/CheckpointLoaderSimple`, { signal: AbortSignal.timeout(5000) });
        if (!resp.ok)
            return false;
...
async function comfyQueuePrompt(workflow) {
    ...
    if (!resp.ok)
        throw new Error(`ComfyUI queue error: ${resp.status} ${await resp.text()}`);
    return (await resp.json()).prompt_id;
}
async function comfyWaitForCompletion(promptId) {
...
function extractVideoFile(outputs) {
...
export async function generateVideoComfyUI(prompt, refImageBuffers) {
    if (!(await comfyuiAvailable()))
        throw new Error('ComfyUI is not running. Open ComfyUI Desktop first.');
    ...
    const vars = { POSITIVE: effectivePrompt, NEGATIVE: negative, SEED: seed };
    // Try I2V workflows first when we have a start frame
    if (startFrameBuffer) {
        for (const def of I2V_WORKFLOWS) {
            if (!(await comfyCheckpointAvailable(def.checkpoint)))
                continue;
            log(`Using ${def.label}`);
            ...
            if (!videoFile)
                throw new Error('ComfyUI returned no video output');
            ...
            if (!videoResp.ok)
                throw new Error(`Download failed: ${videoResp.status}`);
    ...
    throw new Error('No supported video model found in ComfyUI.\n\nDownload ltx-video-2b-v0.9.5.safetensors into ComfyUI/models/checkpoints/');
}
// ---------------------------------------------------------------------------
// OllamaDiffuser backend
// ---------------------------------------------------------------------------
async function ollamaDiffuserAvailable() {
    try {
...
export async function listOllamaDiffuserModels() {
    try {
        ...
        if (Array.isArray(installed) && installed.length > 0) {
            return installed.filter((m) => typeof m === 'string');
        }
        const available = data.available;
        if (Array.isArray(available)) {
            return available.filter((m) => typeof m === 'string');
        }
...
export async function listOllamaDiffuserVideoModels() {
...
async function detectOllamaDiffuserVideoModel() {
    const models = await listOllamaDiffuserModels();
    return models.find((name) => VIDEO_KEYWORDS.some((k) => name.toLowerCase().includes(k))) ?? null;
}
export async function generateVideoOllamaDiffuser(prompt, refImageBase64) {
    ...
    if (!videoModel)
        throw new Error('No video model found in OllamaDiffuser.');
    log(`OllamaDiffuser: ${videoModel}`);
    const body = { prompt, model: videoModel, num_inference_steps: 30 };
    if (refImageBase64)
    ...
    if (!b64)
        throw new Error('OllamaDiffuser returned no video');
    return { buffer: Buffer.from(b64, 'base64'), label: `OllamaDiffuser (${videoModel})` };
}
export async function generateVideo(prompt, backend = 'auto', options) {
    ...
    if (backend === 'comfyui' || backend === 'auto') {
        try {
            ...
            if (refImageBuffers.length > 1)
                effectivePrompt = `[${refImageBuffers.length - 1} extra image(s) described as context] ${prompt}`;
