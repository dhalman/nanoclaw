...
import { generateVideo, hasReferenceVideo, listComfyVideoModels, listOllamaDiffuserVideoModels } from './video.js';
import { generateImage, loadReferenceImages } from './image.js';
...
function setGroupPref(key, value) {
    const prefs = loadPreferences();
...
const buildId = (() => {
    try {
        return fs.readFileSync(BUILD_ID_FILE, 'utf-8').trim();
    }
    catch {
        return '??????';
    }
...
export function shouldThinkFallback(text) {
    return THINK_PATTERN.test(text) || DECISION_PATTERN.test(text);
}
export function shouldEscalateFallback(text) {
    return ESCALATE_PATTERN.test(text);
}
export function selectModelFallback(text, hasImages) {
    if (hasImages)
...
export function detectTaskType(text) {
    ...
    if (CREATIVE_PATTERN.test(text))
        return 'creative';
    if (BRAINSTORM_PATTERN.test(text))
        return 'brainstorm';
    if (ANALYSIS_PATTERN.test(text))
        return 'analysis';
    return 'analysis';
}
export function detectRichTaskType(text) {
    ...
    if (CREATIVE_PATTERN.test(text))
        return 'creative';
    if (RESEARCH_PATTERN.test(text) || detectNeedsWeb(text))
        return 'research';
...
export function getTemperature(text) {
    return TASK_TEMPERATURE[detectTaskType(text)] ?? 0.3;
}
...
export function detectDissatisfaction(text) {
    return DISSATISFACTION_PATTERN.test(text);
}
export function detectNeedsWeb(text) {
...
export function estimateComplexity(text) {
    ...
    if (words < 8 && !hasMultipleQuestions && !hasCodeBlock)
        return 'low';
    if (words > 100 || hasMultipleQuestions || hasCodeBlock || hasListOrSteps)
        return 'high';
    return 'medium';
}
// Build a routing hint string for the coordinator's context
export function buildRouteHint(text, hasImages) {
    ...
    const parts = [];
    if (taskType === 'code')
        parts.push('code task — consider delegating to coder for implementation');
    else if (taskType === 'creative')
        parts.push('creative task');
    else if (taskType === 'brainstorm')
        parts.push('brainstorm');
    else
        parts.push('general');
    parts.push(complexity);
    if (needsWeb)
        parts.push('needs live data — call web_search before answering');
    if (think)
        parts.push('complex reasoning — consider thinking mode or analyst tier');
    if (hasImages)
        parts.push('images attached — vision task');
    if (detectDissatisfaction(text))
        parts.push('USER DISSATISFIED with previous response — escalate to a higher tier or change approach entirely');
    if (shouldEscalateFallback(text))
        parts.push('USER REQUESTS ESCALATION — use the escalate tool to route to deepseek-r1:70b immediately');
    return `[Route: ${parts.join(' · ')}]`;
}
export { TASK_TEMPERATURE };
function loadSecretaryFeedback() {
    try {
...
function appendSecretaryFeedback(grade) {
    try {
...
function formatFeedbackForPrompt(grades) {
...
export async function classifyMessage(text, hasImages) {
    ...
    if (process.env.DISABLE_SECRETARY === '1') {
        ...
        const complexity = escalate ? 'high' : estimateComplexity(text);
        const needsWeb = detectNeedsWeb(text);
        ...
        if (escalate)
            logPerf({ type: 'escalation', buildId, timestamp: new Date().toISOString(), reason: 'user_escalation' });
        if (dissatisfied)
...
Message: ${JSON.stringify(text.slice(0, 400))}`;
    try {
        ...
        if (!resp.ok)
            throw new Error(`classify: ${resp.status}`);
        ...
        const RICH_TYPES = ['chat', 'code', 'creative', 'analysis', 'decision', 'debug', 'research'];
        const taskTypeRich = RICH_TYPES.includes(c.task_type)
            ? c.task_type : 'analysis';
        const TASK_TYPE_MAP = {
        ...
        const complexity = (['low', 'medium', 'high'].includes(c.complexity) ? c.complexity : 'medium');
...
function getStatusLabel(model, think) {
    if (model === MODELS.ARCHITECT)
        return 'Reasoning';
    if (think)
        return 'Thinking';
    if (model === MODELS.CODER)
        return 'Working';
    return 'Responding';
}
...
function saveLatestImage(images) {
    try {
...
function getHistoryConfig(model, think) {
...
async function compressHistory(history, compressThreshold = HISTORY_COMPRESS_THRESHOLD, keepRecent = HISTORY_KEEP_RECENT) {
 ...
 */
function trimHistoryForInference(history) {
...
function splitBySender(prompt) {
    ...
    if (senders.length <= 1)
        return [{ sender: senders[0] || 'unknown', prompt }];
 ...
 */
async function trySecretaryDirect(prompt, cls, chatJid, groupFolder) {
    ...
    if (LANG_REGISTER.test(userText)) {
        ...
        if (found.length > 0) {
            ...
            if (Array.isArray(existing))
                current = existing.filter((l) => typeof l === 'string');
            ...
            const codeToName = (c) => Object.entries(knownLangs).find(([, v]) => v === c)?.[0] || c;
 ...
 */
async function translateForListeners(userText, senderName, chatJid) {
    ...
    try {
        ...
        if (resp.ok) {
            ...
            for (const line of output.split('\n')) {
                ...
                const matchedLang = langs.find((l) => getLanguageNameLocal(l).toLowerCase() === langName.toLowerCase());
...
function readGroupFile(groupFolder, filename) {
    const candidates = [
...
export function getSystemPrompt(assistantName, groupFolder) {
...
export async function handleToolCall(toolName, toolArgs, chatJid, groupFolder, setStatus) {
...
• "Help" — this menu`;
    ...
    if (toolName === 'diffuser_list_models') {
        ...
        if (!resp.ok)
            throw new Error(`OllamaDiffuser error: ${resp.status}`);
        const data = await resp.json();
        const installed = Array.isArray(data.installed) ? data.installed.filter((m) => typeof m === 'string') : [];
        const available = Array.isArray(data.available) ? data.available.filter((m) => typeof m === 'string') : [];
        const notInstalled = available.filter((m) => !installed.includes(m));
        const lines = [];
        if (installed.length)
            lines.push('*Installed:*\n' + installed.map((m) => `• ${m}`).join('\n'));
        if (notInstalled.length)
            lines.push('*Available to download:*\n' + notInstalled.map((m) => `• ${m}`).join('\n'));
        return lines.join('\n\n') || 'No models found.';
    }
    if (toolName === 'comfyui_list_models') {
        ...
        try {
            ...
            log(`ComfyUI checkpoints query failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        // UNETs (FLUX)
        try {
            ...
            log(`ComfyUI UNETs query failed: ${err instanceof Error ? err.message : String(err)}`);
        }
        // LoRAs
        try {
            ...
            log(`ComfyUI LoRAs query failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    ...
    if (toolName === 'preferences') {
        ...
        if (action === 'set') {
            ...
            if (scope === 'user') {
                if (!userId)
                    return 'Error: user_id is required for user scope';
    ...
    if (toolName === 'perf_summary') {
        ...
        if (summary.entries === 0)
            return `No performance data for v${version}.`;
        const lines = [
        ...
        const breakdown = summary.modelBreakdown;
    ...
    if (toolName === 'manage_service') {
        ...
        if (setStatus)
            setStatus(`_${action === 'restart' ? 'Restarting' : 'Starting'} ${service}..._`);
        try {
            fs.mkdirSync(IPC_TASKS_DIR, { recursive: true });
            fs.writeFileSync(path.join(IPC_TASKS_DIR, `svc-${Date.now()}.json`), JSON.stringify({ type: 'manage_service', service, action }));
        }
        catch (err) {
    ...
    if (toolName === 'service_status') {
        ...
        const status = (r) => r.status === 'fulfilled' && r.value ? '🟢 online' : '🔴 offline';
        return [
    ...
    if (toolName === 'comfyui_install_model') {
        ...
        if (!url || !filename || !savePath)
            return 'Error: url, filename, and save_path are all required';
        const comfyHost = process.env.COMFYUI_HOST || 'http://host.docker.internal:8000';
        if (setStatus)
            setStatus(`_Installing ${filename}..._`);
    ...
    if (toolName === 'ollama_generate') {
        ...
        if (isImageModel(model)) {
            ...
            const backend = toolArgs.backend ?? 'auto';
            if (setStatus)
                setStatus(`_Generating image..._`);
            const { buffer, source } = await withTimeout(generateImage(model, prompt, backend, { useReference, embellish }), IMAGE_TOOL_TIMEOUT_MS, `generateImage(${model})`);
            // Send enhanced prompt to chat
            const imageBase64 = buffer.toString('base64');
            fs.mkdirSync(IPC_MSG_DIR, { recursive: true });
            fs.writeFileSync(path.join(IPC_MSG_DIR, `img-${Date.now()}.json`), JSON.stringify({
                type: 'image',
                chatJid,
                imageBase64,
        ...
        const resp = await withTimeout(fetch(`${OLLAMA_HOST}/api/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(body),
        }), TOOL_TIMEOUT_MS, `ollama_generate(${model})`);
        if (!resp.ok)
            throw new Error(`Sub-model error: ${resp.status}`);
        const data = await resp.json();
    ...
    if (toolName === 'generate_art') {
        ...
        if (setStatus)
            setStatus('_Consulting Artist..._');
...
{"prompt": "...", "backend": "auto|comfyui|ollamadiffuser", "use_reference": true|false, "note": "optional"}`;
        ...
        const artistResp = await withTimeout(fetch(`${OLLAMA_HOST}/api/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        }), TOOL_TIMEOUT_MS, 'generate_art(artist)');
        if (!artistResp.ok)
            throw new Error(`Artist error: ${artistResp.status}`);
    ...
    if (toolName === 'generate_film') {
        ...
        if (!request)
            return 'Error: no request provided';
        if (setStatus)
            setStatus('_Consulting Cinematographer..._');
        const refImages = loadReferenceImages();
        const hasRef = refImages && refImages.length > 0;
...
{"prompt": "...", "backend": "auto|comfyui|ollamadiffuser", "use_reference": true|false, "note": "optional"}`;
        ...
        const cinemaResp = await withTimeout(fetch(`${OLLAMA_HOST}/api/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        }), TOOL_TIMEOUT_MS, 'generate_film(cinematographer)');
        if (!cinemaResp.ok)
            throw new Error(`Cinematographer error: ${cinemaResp.status}`);
        const cinemaData = await cinemaResp.json();
    ...
    if (toolName === 'generate_video') {
        ...
        if (setStatus)
            setStatus(`_Generating video..._`);
        const { buffer, source, usedContext, effectivePrompt } = await withTimeout(generateVideo(prompt, backend, { useReference }), VIDEO_TOOL_TIMEOUT_MS, `generateVideo(${backend})`);
    ...
    if (toolName === 'web_search') {
        ...
        const resp = await withTimeout(fetch(url, { headers: { Accept: 'application/json' } }), WEB_TOOL_TIMEOUT_MS, 'web_search');
        if (!resp.ok)
    ...
    if (toolName === 'fetch_url') {
        ...
        if (setStatus)
            setStatus('_Fetching page..._');
        // Jina reader converts any URL to clean markdown
        try {
            const resp = await withTimeout(fetch(`https://r.jina.ai/${url}`, {
                headers: { Accept: 'text/plain', 'X-Retain-Images': 'none' },
            }), WEB_TOOL_TIMEOUT_MS, 'fetch_url(jina)');
            if (resp.ok) {
        ...
        const resp = await withTimeout(fetch(url, { headers: { 'User-Agent': 'Mozilla/5.0 (compatible; Jarvis/1.0)' } }), WEB_TOOL_TIMEOUT_MS, 'fetch_url(direct)');
        if (!resp.ok)
    ...
    if (toolName === 'schedule_task') {
        ...
        if (action === 'create') {
            ...
            if (!prompt || !scheduleType || !scheduleValue)
                return 'Error: create requires prompt, schedule_type, and schedule_value';
        ...
        if (!taskId)
            return `Error: task_id required for ${action}`;
        const ipcTypeMap = { cancel: 'cancel_task', pause: 'pause_task', resume: 'resume_task' };
...
export function getEscalationTier(model, think) {
    if (model === MODELS.CODER)
        return { model: MODELS.COORDINATOR, think: true }; // coder → analyst
    if (model === MODELS.COORDINATOR && !think)
        return { model: MODELS.COORDINATOR, think: true }; // fast → analyst
    if (model === MODELS.COORDINATOR && think)
        return { model: MODELS.ARCHITECT, think: false }; // analyst → architect
    return null; // at max (architect)
}
export async function callOllama(model, messages, chatJid, groupFolder, images, temperature, setStatus, think, onToolStart, complexity) {
    ...
    if (images && images.length > 1 && model === MODELS.VISION) {
        ...
        const descResults = await Promise.all(images.map((img, i) => withTimeout(fetch(`${OLLAMA_HOST}/api/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                model,
        ...
        }), 30_000, `describe-image-${i + 1}`).then(async (r) => {
            if (!r.ok)
                return `Image ${i + 1}: (description unavailable)`;
        ...
        resolvedMessages = messages.map((m, i) => i === messages.length - 1 && m.role === 'user'
    ...
    const messagesWithImages = resolvedImages && resolvedImages.length > 0
        ? resolvedMessages.map((m, i) => i === resolvedMessages.length - 1 && m.role === 'user'
    ...
    if (temperature !== undefined && !(isCoordinator && think) && !isArchitect && !isCoder)
        modelOpts.temperature = temperature;
    const perfStart = Date.now();
    const roundTimings = [];
    const response = await withTimeout(fetch(`${OLLAMA_HOST}/api/chat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            model,
    ...
    }), TOOL_TIMEOUT_MS, `callOllama(${model})`);
    if (!response.ok) {
    ...
    let rounds = 0;
    const commandOutputs = []; // collect run_command results to append verbatim
    while (rounds++ < 5) {
        ...
        for (const tc of msg.tool_calls) {
            ...
            if (tc.function.name === 'escalate' || tc.function.name === 'delegate_to_27b') {
                ...
                if (next) {
                    ...
                    if (onToolStart)
                        onToolStart('escalate');
                    if (setStatus)
                        setStatus(`Escalating to ${next.model}...`);
            ...
            try {
                result = await handleToolCall(tc.function.name, tc.function.arguments, chatJid, groupFolder, setStatus);
            }
            catch (err) {
        ...
        const followUp = await withTimeout(fetch(`${OLLAMA_HOST}/api/chat`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                model,
                messages: [...currentMessages, msg, ...toolResults],
        ...
        }), TOOL_TIMEOUT_MS, `callOllama follow-up(${model})`);
        const next = await followUp.json();
    ...
    if (AUTO_ESCALATE_PATTERN.test(modelResponse)) {
        ...
        if (nextTier) {
            ...
            if (onToolStart)
                onToolStart('escalate');
            if (setStatus)
                setStatus(`Escalating to ${nextTier.model}...`);
 ...
 */
async function runWithConcurrentPoll(mainTask, ctx, chatJid, groupFolder, recentHistory, assistantNameLocal, sessionIdLocal) {
    const interrupts = [];
    let finished = false;
    ...
    const pollLoop = async () => {
        ...
        while (!finished) {
            ...
            for (const msg of messages) {
                ...
                try {
                    const quickReply = await callOllama(MODELS.SECRETARY, [
...
async function main() {
    ...
    const startBackgroundInit = () => {
        ...
        const makeServiceMonitor = (name, check, initialState, onDown, onUp) => {
            ...
            const poll = async () => {
                try {
                    ...
                    if (nowOnline !== state) {
                        ...
                        if (text)
                            sendIpc(`health-${Date.now()}.json`, { type: 'message', chatJid, text });
        ...
        (async () => {
            ...
            try {
                ...
                if (fs.existsSync(buildStatusFile)) {
                    const bs = JSON.parse(fs.readFileSync(buildStatusFile, 'utf-8'));
                    if (bs.status === 'failed') {
            ...
            makeServiceMonitor('Ollama', async () => { const r = await fetch(`${OLLAMA_HOST}/api/tags`, { signal: AbortSignal.timeout(5000) }); return r.ok; }, ollamaOnline, `_${assistantName} offline — Ollama became unreachable._`, `_${assistantName} back online — Ollama reconnected._`);
            makeServiceMonitor('ComfyUI', async () => { const r = await fetch(`${COMFYUI_HOST}/system_stats`, { signal: AbortSignal.timeout(5000) }); return r.ok; }, comfyOnline, `_ComfyUI went offline._`, ``);
            makeServiceMonitor('OllamaDiffuser', async () => { const r = await fetch(`${OLLAMADIFFUSER_HOST}/api/models`, { signal: AbortSignal.timeout(5000) }); return r.ok; }, odOnline, `_OllamaDiffuser went offline._`, ``);
            // Check video backends once — no polling. If not available now, report and move on.
            const [comfyModels, odModels] = await Promise.all([
                listComfyVideoModels().catch(() => []),
                listOllamaDiffuserVideoModels().catch(() => []),
            ]);
            comfyOnline = comfyModels.length > 0;
            odOnline = odModels.length > 0;
            const failures = [];
            if (!comfyOnline)
                failures.push('ComfyUI');
            if (!odOnline)
                failures.push('OllamaDiffuser');
    ...
    let currentImages = containerInput.images?.length
    ...
    const systemMsg = { role: 'system', content: getSystemPrompt(assistantName, groupFolder) };
    ...
    try {
        while (true) {
            ...
            for (const senderGroup of senderGroups) {
                ...
                const routeMsg = { role: 'system', content: routeHint };
                ...
                fs.writeFileSync(path.join(IPC_MSG_DIR, `${ackId}-start.json`), JSON.stringify({ type: 'thinking_start', chatJid, text: `_${modelLabel} · ${responseType}_`, thinkingId: ackId }));
                let thinkingInterval = null;
                const setStatus = (text) => {
                    if (/generating\s+image/i.test(text)) {
                        statusLabel = 'Processing';
                        responseType = 'image';
                    }
                    if (/generating\s+video/i.test(text)) {
                        statusLabel = 'Processing';
                        responseType = 'video';
                    }
                    elapsedSeconds += 5;
                ...
                const clearThinking = () => {
                    ...
                    fs.writeFileSync(path.join(IPC_MSG_DIR, `${ackId}-zzz-clear.json`), JSON.stringify({ type: 'thinking_clear', thinkingId: ackId }));
                };
                ...
                const callPromise = callOllama(model, messages, chatJid, groupFolder, currentImages, temperature, setStatus, thinking, (toolName) => { taskPhaseRef.value = `tool:${toolName}`; }, cls.complexity);
                const taskCtx = {
                    description: prompt.slice(0, 120).replace(/\n/g, ' '),
                    getPhase: () => taskPhaseRef.value,
                    startedAt: responseStart,
                };
                const { response, interrupts } = await runWithConcurrentPoll(callPromise, taskCtx, chatJid, groupFolder, history, assistantName, sessionId);
