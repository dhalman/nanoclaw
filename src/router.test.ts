...
describe('formatOutbound', () => {
  it('strips internal tags and returns text', () => {
    expect(formatOutbound('Hello<internal>reasoning</internal> world')).toBe(
      'Hello world',
    );
  });
...
describe('routeOutbound', () => {
  ...
  it('throws when no channel matches', () => {
    const tg = makeChannel('telegram', 'tg:');
    expect(() => routeOutbound([tg], 'dc:123', 'hello')).toThrow(
      'No channel for JID',
    );
  });
  ...
  it('skips disconnected channels', () => {
    ...
    expect(() => routeOutbound([tg], 'tg:123', 'hello')).toThrow(
      'No channel for JID',
    );
  });
});
