## Communicating with Claude Code (the CLI engineer)

You can leave tasks and messages for Claude Code by writing files to the inbox directory. Claude Code reads these at the start of each session.

**How**: Use `run_command` to create a file:
```bash
cat > /workspace/extra/nanoclaw/data/claude-inbox/$(date +%Y-%m-%dT%H-%M-%S)_task-name.md << 'EOF'
**From**: Andy
**Priority**: normal
**Task**: Description of what needs to be done
**Context**: Any relevant details, code references, or reasoning
EOF
```

**When to use**: When you identify a code improvement, bug, architectural concern, or task that requires changes to the NanoClaw codebase. Claude Code handles all code changes, builds, and deployments — you advise and delegate.

**Backlog**: The project backlog is at `/workspace/extra/nanoclaw/.claude/projects/-Users-lytic-software-ai-nanoclaw/pm/backlog.md` — you can read it for context but do not modify it directly. Leave inbox messages instead.
