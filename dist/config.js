...
export const DISABLE_SECRETARY = (process.env.DISABLE_SECRETARY || envConfig.DISABLE_SECRETARY) === '1';
...
export const TRIGGER_PATTERN = new RegExp(`(?:^|\\b(?:hey|hi|ok|yo)\\s+)${escapeRegex(ASSISTANT_NAME)}\\b[,?!:]?(?:\\s|$)|` + // "Jarvis, ..." / "hey Jarvis" / "hi Jarvis"
...
export const DISMISS_PATTERN = /^\s*(?:bye|goodbye|go away|stop|leave|dismiss|shut up|quiet|enough|done|no thanks?|nah|nope|not now|i'?m good|we'?re good|that'?s (?:all|enough|it)|never\s?mind|whatever|ok bye|k bye|👋)\s*[.!]?\s*$/i;
// Cancel command: kill the active container immediately
export const CANCEL_PATTERN = /^\s*(\/stop|\/cancel|stop|cancel|nevermind)\s*$/i;
