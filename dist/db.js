...
export function setLastGroupSync() {
    ...
    stmt(`INSERT OR REPLACE INTO chats (jid, name, last_message_time) VALUES ('__group_sync__', '__group_sync__', ?)`).run(now);
}
/**
...
export function storeMessage(msg) {
    stmt(`INSERT OR REPLACE INTO messages (id, chat_jid, sender, sender_name, content, timestamp, is_from_me, is_bot_message) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(msg.id, msg.chat_jid, msg.sender, msg.sender_name, msg.content, msg.timestamp, msg.is_from_me ? 1 : 0, msg.is_bot_message ? 1 : 0);
}
/**
...
export function getMessageById(chatJid, messageId) {
    const row = db
        .prepare('SELECT id, chat_jid, sender, sender_name, content, timestamp, is_from_me FROM messages WHERE chat_jid = ? AND id = ?')
...
export function getTaskById(id) {
    return stmt('SELECT * FROM scheduled_tasks WHERE id = ?').get(id);
}
...
export function updateTask(id, updates) {
    ...
    if (fields.length === 0)
        return;
    values.push(id);
    stmt(`UPDATE scheduled_tasks SET ${fields.join(', ')} WHERE id = ?`).run(...values);
}
...
export function setRouterState(key, value) {
    stmt('INSERT OR REPLACE INTO router_state (key, value) VALUES (?, ?)').run(key, value);
}
// --- Session accessors ---
...
export function setSession(groupFolder, sessionId) {
    stmt('INSERT OR REPLACE INTO sessions (group_folder, session_id) VALUES (?, ?)').run(groupFolder, sessionId);
}
...
export function setRegisteredGroup(jid, group) {
    ...
    stmt(`INSERT OR REPLACE INTO registered_groups (jid, name, folder, trigger_pattern, added_at, container_config, requires_trigger, is_main)
...
export function getAllRegisteredGroups() {
    const rows = stmt('SELECT * FROM registered_groups').all();
    const result = {};
    for (const row of rows) {
        if (!isValidGroupFolder(row.folder)) {
            logger.warn({ jid: row.jid, folder: row.folder }, 'Skipping registered group with invalid folder');
