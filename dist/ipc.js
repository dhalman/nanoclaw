...
import { sendJarvisMessage, sendJarvisPhoto, sendJarvisVideo, sendPoolMessage, editJarvisMessage, deleteJarvisMessage, pinJarvisMessage, } from './channels/telegram.js';
    ...
    if (!_statusEntries) {
        try {
            ...
            if (raw) {
                ...
                _statusEntries = new Map(Object.entries(parsed).map(([k, v]) => [k, v]));
            }
            else {
...
function saveStatusEntries() {
    setRouterState('last_status_message_ids', JSON.stringify(Object.fromEntries(getStatusEntries())));
}
/** Call when a user message is received for a chatJid (no-op — status is always edit-in-place now). */
export function markUserActivity(_chatJid) {
 ...
 */
export async function sendOrEditStatus(chatJid, text) {
    const entries = getStatusEntries();
    const entry = entries.get(chatJid);
    if (entry?.messageId) {
        try {
            await editJarvisMessage(chatJid, entry.messageId, text);
            logger.info({ chatJid, messageId: entry.messageId }, 'Status message updated');
            return;
        }
        catch (err) {
 ...
 */
export async function sendStoppedStatus(registeredGroups, assistantName) {
                    ...
                    for (const file of messageFiles) {
                        ...
                        try {
                            ...
                            if (expectedBuildId &&
                                ...
                                logger.debug({ file, dataBuildId: data.buildId, expectedBuildId }, 'Dropping IPC message from old image');
                                safeUnlink(filePath);
                                continue;
                            }
                            if (data.type === 'message' && data.chatJid && data.text) {
                                ...
                                if (isMain ||
                                    ...
                                    if (sourceGroup === 'telegram_ollama' &&
                                ...
                                if (isMain ||
                                    ...
                                    for (const [tid, entry] of thinkingMessages) {
                                        if (entry.chatJid === data.chatJid) {
                                            ...
                                            logger.info({ chatJid: entry.chatJid, thinkingId: tid }, 'Cleared orphaned thinking message');
                                        }
                                    }
                                ...
                                if (isMain ||
                                    ...
                                    if (data.chatJid.startsWith('tg:') ||
                                        ...
                                        logger.warn({ sourceGroup }, 'Image IPC to non-Telegram target — not yet supported');
                                    }
                                ...
                                if (isMain ||
                                    (targetGroup && targetGroup.folder === sourceGroup)) {
                                    await sendJarvisVideo(data.chatJid, data.videoBase64, data.caption);
                                    logger.info({ chatJid: data.chatJid, sourceGroup }, 'IPC video sent');
                                }
                                else {
                                ...
                                if (msgId !== null) {
                                    ...
                                    logger.debug({ thinkingId: data.thinkingId, msgId }, 'Thinking message sent');
                                }
                            }
                                ...
                                if (entry) {
                                    await editJarvisMessage(entry.chatJid, entry.messageId, data.text);
                                }
                            }
                                ...
                                if (entry) {
                                    ...
                                    logger.debug({ thinkingId: data.thinkingId }, 'Thinking message cleared');
                                }
                            }
                            safeUnlink(filePath);
                        }
...
deps) {
    ...
    switch (data.type) {
        ...
        case 'rebuild':
            ...
            logger.info({ sourceGroup }, 'Rebuild requested via IPC — spawning build.sh');
            {
        ...
        case 'manage_service': {
            ...
            if (!validServices.includes(service)) {
                logger.warn({ service, sourceGroup }, 'Invalid service name in manage_service');
                break;
            }
            if (!validActions.includes(action)) {
                logger.warn({ action, sourceGroup }, 'Invalid action in manage_service');
                break;
            }
            logger.info({ service, action, sourceGroup }, 'Service management requested via IPC');
