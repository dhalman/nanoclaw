...
export class GroupQueue {
    ...
    killActive(groupJid) {
        ...
        logger.info({ groupJid, containerName: name }, 'Container killed by user request');
        return true;
    }
    /**
     ...
     */
    async prespawn(groupJid, groupFolder, spawnFn) {
        const state = this.getGroup(groupJid);
        if (state.active || this.shuttingDown)
            return;
        state.active = true;
        ...
        state.groupFolder = groupFolder;
        this.activeCount++;
        logger.info({ groupJid, activeCount: this.activeCount }, 'Pre-spawning container');
        try {
            await spawnFn();
        }
        catch (err) {
            logger.error({ groupJid, err }, 'Pre-spawn container error');
        }
        finally {
            state.active = false;
            state.process = null;
            ...
            this.activeCount--;
            // Drain pending messages that arrived during the container's lifetime or restart gap
