...
function getGroupPref(groupFolder, key) {
    try {
...
function getUserPreferredLanguages(groupFolder, userId) {
    try {
        const prefsPath = path.join(GROUPS_DIR, groupFolder, '.preferences.json');
        if (!fs.existsSync(prefsPath))
            return ['en'];
        ...
        if (groupLang && typeof groupLang === 'string')
            return [groupLang];
        return ['en'];
    }
    catch {
...
function getTranslatorLanguages(groupFolder) {
 ...
 */
export async function initBotPool(tokens) {
    for (const token of tokens) {
        try {
            const api = new Api(token);
            const me = await api.getMe();
            poolApis.push(api);
            logger.info({ username: me.username, id: me.id, poolSize: poolApis.length }, 'Pool bot initialized');
        }
        catch (err) {
 ...
 */
export async function sendPoolMessage(chatId, text, sender, groupFolder) {
    if (poolApis.length === 0) {
...
let jarvisApi = null;
let jarvisBot = null;
export async function initJarvisBot(token, opts) {
    try {
        ...
        jarvisBot.on('message:text', async (ctx) => {
            ...
            if (ctx.message.reply_to_message &&
                ...
                if (isTranslateComplaint) {
                    ...
                    const origReply = ctx.message.reply_to_message
                        ?.reply_to_message;
                    sourceText = origReply?.text || origReply?.caption;
                    if (origReply?.message_id)
                        replyTarget = origReply.message_id;
                ...
                if (sourceText) {
                    ...
                    if (targetLangs.length === 0) {
                        sendJarvisMessage(chatJid, '_No languages registered. Tell me which language to translate to, e.g. "Jarvis, translate this to Spanish"_').catch(() => { });
                        return;
                    }
            ...
            const isReplyToJarvis = ctx.message.reply_to_message &&
                ctx.message.reply_to_message.from?.id === jarvisBot?.botInfo?.id;
            const messageContent = isReplyToJarvis && !text.includes(ASSISTANT_NAME)
                ? `${ASSISTANT_NAME}, ${text}`
                : text;
            ...
            logger.info({ chatJid, chatName, sender: senderName }, 'Jarvis message received');
        });
        jarvisBot.on('message:video', async (ctx) => {
            ...
            if (video && (!video.file_size || video.file_size <= MAX_VIDEO_BYTES)) {
                ...
                logger.warn({ chatJid, size: video.file_size }, 'Jarvis video too large to download (>20MB)');
            }
            ...
            logger.info({ chatJid, sender: senderName, hasVideo: !!videoBase64 }, 'Jarvis video received');
        });
        jarvisBot.on('message:photo', async (ctx) => {
            ...
            logger.info({ chatJid, sender: senderName, hasImage: images.length > 0 }, 'Jarvis photo received');
        });
        jarvisBot.on('message:voice', async (ctx) => {
            ...
            try {
                ...
                if (file.file_path) {
                    ...
                    if (resp.ok) {
                        ...
                        if (result) {
                            ...
                            logger.info({ chatJid, chars: result.text.length, lang: result.language }, 'Jarvis voice message transcribed');
                            ...
                            const translations = await translateToMultiple(result.text, result.language, targetLangs);
                            if (translations.length > 0) {
                ...
                logger.warn({ chatJid, err }, 'Failed to transcribe Jarvis voice message');
            }
        ...
        jarvisBot.on('message:new_chat_members', async (ctx) => {
            ...
            for (const member of newMembers) {
                ...
                const assistantName = group.containerConfig?.assistantName || ASSISTANT_NAME;
                sendJarvisMessage(chatJid, `_Welcome, ${name}! 👋 I'm ${assistantName}. Say my name to start a conversation with me — I'll stay engaged until you dismiss me. Happy to help anytime!_`).catch(() => { });
        ...
        jarvisBot.on('message_reaction', async (ctx) => {
            ...
            try {
                ...
                const { translateToMultiple, getLanguageName } = await import('../translation.js');
                // Use 'auto' detection — translate to all subscribed languages
                const translations = await translateToMultiple(msg.content, 'auto', targetLangs);
                if (translations.length > 0) {
                    const echo = translations
                        .map((t) => `_🌐 [${t.targetName}] ${t.text}_`)
                        .join('\n\n');
...
export async function sendJarvisMessage(chatId, text, replyToMessageId) {
    if (!jarvisApi) {
    ...
    try {
        ...
        if (text.length <= MAX_LENGTH) {
            firstMessageId = await sendTelegramMessage(jarvisApi, numericId, text, replyOpts);
        }
        else {
            for (let i = 0; i < text.length; i += MAX_LENGTH) {
                const msgId = await sendTelegramMessage(jarvisApi, numericId, text.slice(i, i + MAX_LENGTH));
...
export async function editJarvisMessage(chatId, messageId, text) {
    if (!jarvisApi)
        return;
    const numericId = chatId.replace(/^tg(-j)?:/, '');
    try {
        ...
        logger.debug({ chatId, messageId, err }, 'editJarvisMessage failed (ignored)');
    }
}
export async function deleteJarvisMessage(chatId, messageId) {
    if (!jarvisApi)
        return;
    const numericId = chatId.replace(/^tg(-j)?:/, '');
    try {
        ...
        logger.debug({ chatId, messageId, err }, 'deleteJarvisMessage failed (ignored)');
    }
}
export async function pinJarvisMessage(chatId, messageId) {
    if (!jarvisApi)
        return;
    const numericId = chatId.replace(/^tg(-j)?:/, '');
    try {
...
export async function unpinJarvisMessage(chatId, messageId) {
    if (!jarvisApi)
        return;
    const numericId = chatId.replace(/^tg(-j)?:/, '');
    try {
 ...
 */
export async function sendJarvisVideo(chatId, videoBase64, caption) {
    if (!jarvisApi) {
    ...
    try {
        ...
        logger.info({ chatId, captionLen: caption?.length ?? 0 }, 'Jarvis video sent');
    }
    catch (err) {
 ...
 */
export async function sendJarvisPhoto(chatId, imageBase64, caption) {
    if (!jarvisApi) {
            ...
            try {
                ...
                if (file.file_path) {
                    ...
                    if (resp.ok) {
                        ...
                        if (result) {
                            content = `[Voice: ${result.text}]`;
                            logger.info({ chatJid, chars: result.text.length, lang: result.language }, 'Voice message transcribed');
                            const targetLangs = isGroup
                                ? getTranslatorLanguages(group.folder)
                                : getUserPreferredLanguages(group.folder, ctx.from?.id?.toString() || '');
                            let echo = `_🎙 [${getLanguageName(result.language)}] "${result.text}"_`;
                            const translations = await translateToMultiple(result.text, result.language, targetLangs);
                            if (translations.length > 0) {
                                for (const t of translations) {
