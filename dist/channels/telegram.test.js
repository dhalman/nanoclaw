...
// Mock transcription so tests don't spawn Python
...
vi.mock('../config.js', () => ({
    ASSISTANT_NAME: 'Andy',
    TRIGGER_PATTERN: /(?:^|\b(?:hey|hi|ok|yo)\s+)Andy\b[,?!:]?\s|(?:^|\s)@Andy\b|^Andy[,?!:\s]/im,
}));
// Mock logger
vi.mock('../logger.js', () => ({
    logger: {
...
function createVoiceCtx(overrides = {}) {
    ...
    return {
        ...
        message: {
            ...
            message_id: overrides.messageId ?? 1,
            caption: undefined,
            voice: { file_id: overrides.fileId ?? 'voice-file-123' },
        },
        me: { username: 'andy_ai_bot' },
    };
}
...
describe('TelegramChannel', () => {
    ...
    describe('voice transcription', () => {
        ...
        it('transcribes voice message and uses transcript as content', async () => {
            ...
            const channel = new TelegramChannel('test-token', opts);
            await channel.connect();
            await triggerMediaMessage('message:voice', createVoiceCtx());
            expect(opts.onMessage).toHaveBeenCalledWith('tg:100200300', expect.objectContaining({ content: '[Voice: hello world]' }));
        });
        it('echoes transcript back to chat after transcription', async () => {
            vi.mocked(transcribeAudio).mockResolvedValue({
                text: 'pick up milk',
                language: 'en',
            ...
            await channel.connect();
            await triggerMediaMessage('message:voice', createVoiceCtx());
            expect(currentBot().api.sendMessage).toHaveBeenCalledWith('100200300', '_🎙 [English] "pick up milk"_', { parse_mode: 'Markdown' });
        });
        it('uses unavailable placeholder when transcribeAudio returns null', async () => {
        ...
        it('does not echo when transcription returns null', async () => {
            vi.mocked(transcribeAudio).mockResolvedValue(null);
            const opts = createTestOpts();
            const channel = new TelegramChannel('test-token', opts);
            await channel.connect();
            await triggerMediaMessage('message:voice', createVoiceCtx());
            // sendMessage should not have been called (no echo)
            expect(currentBot().api.sendMessage).not.toHaveBeenCalled();
        });
        it('falls back to [Voice message] when getFile has no file_path', async () => {
            const opts = createTestOpts();
            const channel = new TelegramChannel('test-token', opts);
            await channel.connect();
            currentBot().api.getFile.mockResolvedValueOnce({ file_path: undefined });
            await triggerMediaMessage('message:voice', createVoiceCtx());
            expect(opts.onMessage).toHaveBeenCalledWith('tg:100200300', expect.objectContaining({ content: '[Voice message]' }));
        });
        it('falls back to [Voice message] when audio download fails', async () => {
        ...
        it('falls back to [Voice message] when transcribeAudio throws', async () => {
            vi.mocked(transcribeAudio).mockRejectedValue(new Error('spawn failed'));
            const opts = createTestOpts();
            const channel = new TelegramChannel('test-token', opts);
            await channel.connect();
            await triggerMediaMessage('message:voice', createVoiceCtx());
        ...
        it('emits metadata for voice messages', async () => {
            vi.mocked(transcribeAudio).mockResolvedValue({
                text: 'test',
                language: 'en',
            ...
            await channel.connect();
            await triggerMediaMessage('message:voice', createVoiceCtx());
            expect(opts.onChatMetadata).toHaveBeenCalledWith('tg:100200300', expect.any(String), undefined, 'telegram', true);
        });
        it('ignores voice messages from unregistered chats', async () => {
            ...
            const channel = new TelegramChannel('test-token', opts);
            await channel.connect();
            await triggerMediaMessage('message:voice', createVoiceCtx({ chatId: 999999 }));
            expect(opts.onMessage).not.toHaveBeenCalled();
        });
        it('passes correct file_id to getFile', async () => {
            vi.mocked(transcribeAudio).mockResolvedValue({
                text: 'test',
                language: 'en',
                languageProbability: 0.95,
            ...
            const channel = new TelegramChannel('test-token', opts);
            await channel.connect();
            await triggerMediaMessage('message:voice', createVoiceCtx({ fileId: 'abc-xyz-789' }));
            expect(currentBot().api.getFile).toHaveBeenCalledWith('abc-xyz-789');
        });
    });
    // --- sendMessage ---
