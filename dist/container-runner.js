...
import { CONTAINER_IMAGE, CONTAINER_MAX_OUTPUT_SIZE, CONTAINER_TIMEOUT, CREDENTIAL_PROXY_PORT, DATA_DIR, DISABLE_SECRETARY, GROUPS_DIR, IDLE_TIMEOUT, SEARXNG_PORT, TIMEZONE, } from './config.js';
 ...
 */
function syncIfNewer(srcDir, dstDir, opts = {}) {
    if (!fs.existsSync(srcDir))
        return;
    fs.mkdirSync(dstDir, { recursive: true });
    const entries = fs.readdirSync(srcDir, { withFileTypes: true });
    for (const entry of entries) {
        if (opts.directories && !entry.isDirectory())
            continue;
...
function buildVolumeMounts(group, isMain) {
    ...
    syncIfNewer(path.join(process.cwd(), 'container', 'skills'), path.join(groupSessionsDir, 'skills'), { directories: true });
    mounts.push({
    ...
    syncIfNewer(path.join(projectRoot, 'container', 'agent-runner', 'src'), groupAgentRunnerDir);
    mounts.push({
    ...
    const ollamaRunnerSrc = path.join(projectRoot, 'container', 'ollama-runner', 'src');
    const ollamaRunnerBuildId = path.join(projectRoot, 'container', 'ollama-runner', 'build-id.txt');
    const ollamaRunnerChangelog = path.join(projectRoot, 'container', 'ollama-runner', 'changelog.json');
    const ollamaRunnerTsConfig = path.join(projectRoot, 'container', 'ollama-runner', 'tsconfig.json');
...
function buildContainerArgs(mounts, containerName, ollamaRunner) {
    ...
    return new Promise((resolve) => {
        ...
        container.on('close', (code) => {
            ...
            if (ollamaRunner && code !== 0 && !timedOut) {
                try {
                    const ipcMsgDir = path.join(resolveGroupIpcPath(group.folder), 'messages');
                    fs.mkdirSync(ipcMsgDir, { recursive: true });
                    const assistantName = input.assistantName || 'Jarvis';
                    fs.writeFileSync(path.join(ipcMsgDir, `stopped-${Date.now()}.json`), JSON.stringify({
                        type: 'message',
