...
describe('blocked pattern matching', () => {
    it('blocks .ssh in path components', () => {
        ...
        const match = blocked.find((p) => pathParts.some((part) => part === p || part.includes(p)));
...
describe('container path validation', () => {
    ...
    it('accepts valid relative paths', () => {
        const p = 'myproject';
        expect(!p.includes('..') && !p.startsWith('/') && p.trim() !== '').toBe(true);
    });
});
// --- path expansion ---
...
//# sourceMappingURL=mount-security.test.js.map