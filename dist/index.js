...
import { ASSISTANT_NAME, CANCEL_PATTERN, CREDENTIAL_PROXY_PORT, IDLE_TIMEOUT, POLL_INTERVAL, JARVIS_BOT_TOKEN, TELEGRAM_BOT_POOL, TIMEZONE, } from './config.js';
import { initBotPool, initJarvisBot, reactToMessage, removeReaction, sendJarvisMessage, } from './channels/telegram.js';
import { startCredentialProxy } from './credential-proxy.js';
import { checkEngagement, disengageAll, disengageUser, isEngaged, learnUserEmoji, } from './engagement.js';
...
import { getAllRegisteredGroups, getAllSessions, getMessagesSince, getNewMessages, getRouterState, initDatabase, setRegisteredGroup, setRouterState, setSession, storeChatMetadata, storeMessage, } from './db.js';
import { GroupQueue } from './group-queue.js';
import { resolveGroupFolderPath } from './group-folder.js';
import { markUserActivity, sendOrEditStatus, sendStoppedStatus, startIpcWatcher, } from './ipc.js';
import { findChannel, formatMessages, formatOutbound } from './router.js';
import { getAvailableGroups as getAvailableGroupsFromSnapshots, prepareAndWriteSnapshots, writeGroupsSnapshot, } from './snapshots.js';
...
async function processGroupMessages(chatJid) {
    ...
    const engagement = await checkEngagement(chatJid, group, missedMessages, loadSenderAllowlist());
    ...
    const output = await runAgent(group, prompt, chatJid, async (result) => {
        ...
        if (result.result) {
            ...
            const disengageMatches = text.matchAll(/<disengage(?::([^/]*))?\/?>|<disengage(?::([^>]*))?>(?:<\/disengage>)?/g);
            ...
            if (text) {
                ...
                if (group.containerConfig?.ollamaRunner) {
                    ...
                    if (replyTo)
                        removeReaction(chatJid, replyTo).catch(() => { });
                }
                else {
...
async function runAgent(group, prompt, chatJid, onOutput) {
    ...
    const images = imageAge < PENDING_IMAGE_TTL_MS ? pendingImages[chatJid] : undefined;
 ...
 */
function prespawnGroup(chatJid, group) {
    if (shuttingDown)
        return;
    const doSpawn = async () => {
        const isMain = group.isMain === true;
        ...
        await runContainerAgent(group, {
            ...
            if (output.result) {
                const text = output.result
                    ...
                    .trim();
                if (text) {
                    await sendJarvisMessage(chatJid, text, lastTriggerMessageId[chatJid]);
...
async function main() {
        ...
        onMessage: (chatJid, msg) => {
            ...
            if (msg.images && msg.images.length > 0) {
                ...
                if (group?.containerConfig?.ollamaRunner) {
                    ...
                    fs.promises
                        .writeFile(latestImageFile, JSON.stringify({ images: msg.images, savedAt: Date.now() }))
                        .catch((err) => logger.debug({ chatJid, err }, 'Failed to persist latest image to group folder'));
                }
            }
            ...
            if (msg.videoBase64) {
                ...
                if (group?.containerConfig?.ollamaRunner) {
                    ...
                    Promise.all([
                        ...
                        fs.promises.writeFile(metaPath, JSON.stringify({ savedAt: Date.now() })),
                    ])
                        .then(() => logger.debug({ chatJid }, 'Latest video persisted to group folder'))
                        .catch((err) => logger.debug({ chatJid, err }, 'Failed to persist latest video to group folder'));
                }
            }
        },
        onChatMetadata: (chatJid, timestamp, name, channel, isGroup) => storeChatMetadata(chatJid, timestamp, name, channel, isGroup),
        registeredGroups: () => registeredGroups,
        isUserEngaged: (chatJid, userId) => isEngaged(chatJid, userId),
    };
    ...
    // Send online status from host (bot is initialized, can pin)
    {
        const buildId = (() => {
            try {
                return fs
                    .readFileSync(path.join(process.cwd(), 'container/ollama-runner/build-id.txt'), 'utf-8')
                    .trim();
            }
            catch {
                return '?';
        ...
        for (const [chatJid, group] of Object.entries(registeredGroups)) {
            if (!group.containerConfig?.ollamaRunner)
                continue;
            if (!chatJid.startsWith('tg:') && !chatJid.startsWith('tg-j:'))
                continue;
            const name = group.containerConfig.assistantName || ASSISTANT_NAME;
            sendOrEditStatus(chatJid, `_${name} v${buildId} online — ${onlineTime}_ 😎`)
    ...
    for (const [chatJid] of Object.entries(registeredGroups)) {
        ...
        if (pending.length > 0) {
            ...
            logger.info({ chatJid, skipped: pending.length }, 'Skipped pending messages from before restart');
        }
    }
    saveState();
