...
vi.mock('./container-runtime.js', () => ({
    ...
    stopContainer: vi.fn((name) => `docker stop ${name}`),
}));
// Mock group-folder
vi.mock('./group-folder.js', () => ({
    resolveGroupFolderPath: vi.fn((f) => `/tmp/nanoclaw-test-groups/${f}`),
    resolveGroupIpcPath: vi.fn((f) => `/tmp/nanoclaw-test-data/ipc/${f}`),
}));
...
const jarvisGroup = {
    ...
    trigger: '@Jarvis',
    added_at: new Date().toISOString(),
    containerConfig: { ollamaRunner: true, assistantName: 'Jarvis' },
};
const jarvisInput = {
    prompt: 'Hello',
    groupFolder: 'test-group',
...
describe('container-runner timeout behavior', () => {
    ...
    it('ollama-runner container writes stopped IPC on unexpected exit', async () => {
        ...
        expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('stopped-'), expect.stringContaining('Jarvis stopped'));
    });
